  ...
  drivendata.forEach(function(d){
    it('test1', function(){
      if(d.key == "receive:") {
        ...
        it(d.data.caseName[j], function(done) {
          var deviceType = eval(d.data.caseSummary[j][1]);//主板类型
          var optionsModular = d.data.caseSummary[j][2];//读取的模块类型
          var optionsPort = d.data.caseSummary[j][3];//port、slot口参数
          deviceType.getSensorValue(optionsModular, optionsPort,function(result) {
            eval(d.data.caseSummary[j][4]);//断言
            done();
          });
        });
      }
